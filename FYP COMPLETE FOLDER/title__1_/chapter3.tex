...
\subsubsection{Virtual Reality \& Environment Requirements}
\begin{table}[H]
\centering
\renewcommand{\arraystretch}{1.3}
\caption{Virtual Reality and Environment Control Requirements}
\label{tab:func-vr}
\begin{tabular}{|p{0.1\linewidth}|p{0.85\linewidth}|}
\hline
\rowcolor{gray!15} \textbf{ID} & \textbf{Description} \\ \hline
FR-11 & The system must leverage logic controllers to map detected emotions to specific VR simulations and intensity levels. \\ \hline
FR-12 & The system must dynamically render 3D environments in \textbf{Unity}, adjusting lighting and audio based on AI recommendations. \\ \hline
FR-13 & The system must include an automated failsafe that triggers an emergency protocol if high-risk keywords are detected. \\ \hline
\end{tabular}
\end{table}
...
\begin{tikzpicture}[every node/.style={font=\small}, node distance=2cm, auto]
    ...
    \node[usecase] (UC6) at (4,-2) {UC-06: VR Therapy Playback};
...
\begin{tikzpicture}[every node/.style={font=\small}, node distance=2cm, auto]
    ...
    \node[usecase] (UC12) at (0,3.5) {UC-12: Update Simulation Library};
...
\centering
\renewcommand{\arraystretch}{1.3}
\caption{UC-05: Simulation Recommendation}
\label{tab:uc05}
\begin{tabular}{|p{0.25\linewidth}|p{0.7\linewidth}|}
\hline
\rowcolor{gray!15} \textbf{Field} & \textbf{Details} \\ \hline
\textbf{Use Case ID} & UC-05 \\ \hline
\textbf{Description} & Determines optimal VR environment based on analysis. \\ \hline
\textbf{Actors} & System (DeepSeek Logic) \\ \hline
\textbf{Preconditions} & Root cause and tags identified. \\ \hline
\textbf{Postconditions} & Simulation config sent to Unity. \\ \hline
\textbf{Flow of Events} & 1. DeepSeek analyzes user state. \newline 2. Selects environment/intensity. \newline 3. Constructs JSON payload. \\ \hline
\end{tabular}
\end{table}
...
\centering
\renewcommand{\arraystretch}{1.3}
\caption{UC-06: VR Therapy Playback}
\label{tab:uc06}
\begin{tabular}{|p{0.25\linewidth}|p{0.7\linewidth}|}
\hline
\rowcolor{gray!15} \textbf{Field} & \textbf{Details} \\ \hline
\textbf{Use Case ID} & UC-06 \\ \hline
\textbf{Description} & Renders the selected 3D environment. \\ \hline
\textbf{Actors} & User, Unity Engine \\ \hline
\textbf{Preconditions} & Recommendation received. \\ \hline
\textbf{Postconditions} & User immersed in simulation. \\ \hline
\textbf{Flow of Events} & 1. Unity parses JSON. \newline 2. Loads assets dynamically. \newline 3. User immersed in session. \\ \hline
\end{tabular}
\end{table}
...
\centering
\renewcommand{\arraystretch}{1.3}
\caption{UC-07: Session Timer Management}
\label{tab:uc07}
\begin{tabular}{|p{0.25\linewidth}|p{0.7\linewidth}|}
\hline
\rowcolor{gray!15} \textbf{Field} & \textbf{Details} \\ \hline
\textbf{Use Case ID} & UC-07 \\ \hline
\textbf{Description} & Controls duration of VR experience to ensure safety. \\ \hline
\textbf{Actors} & System \\ \hline
\textbf{Preconditions} & Simulation active. \\ \hline
\textbf{Postconditions} & Session ends gracefully. \\ \hline
\textbf{Flow of Events} & 1. Timer starts. \newline 2. Displays visual timer. \newline 3. Fades out upon expiration. \\ \hline
\end{tabular}
\end{table}
...
\centering
\renewcommand{\arraystretch}{1.3}
\caption{UC-08: Exit Session Anytime}
\label{tab:uc08}
\begin{tabular}{|p{0.25\linewidth}|p{0.7\linewidth}|}
\hline
\rowcolor{gray!15} \textbf{Field} & \textbf{Details} \\ \hline
...
\centering
\renewcommand{\arraystretch}{1.3}
\caption{UC-09: Provide Session Feedback}
\label{tab:uc09}
\begin{tabular}{|p{0.25\linewidth}|p{0.7\linewidth}|}
\hline
\rowcolor{gray!15} \textbf{Field} & \textbf{Details} \\ \hline
...
\centering
\renewcommand{\arraystretch}{1.3}
\caption{UC-10: Calculate Improvement Score}
\label{tab:uc10}
\begin{tabular}{|p{0.25\linewidth}|p{0.7\linewidth}|}
\hline
\rowcolor{gray!15} \textbf{Field} & \textbf{Details} \\ \hline
...
\centering
\renewcommand{\arraystretch}{1.3}
\caption{UC-11: Emergency Escalation}
\label{tab:uc11}
\begin{tabular}{|p{0.25\linewidth}|p{0.7\linewidth}|}
\hline
\rowcolor{gray!15} \textbf{Field} & \textbf{Details} \\ \hline
